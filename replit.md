...
## Recent Changes
- **2026-05-14**: Session-to-session comparison in workout history — SessionDetailScreen now shows a compact "vs last time" card below the meta card when a previous session with the same template exists; card shows Duration (shorter = green), Volume in kg (higher = green), and Exercises logged (more = green) deltas with trending-up/down icons; previous session found by filtering all sessions on `sessionTemplateId`, excluding current, sorting by `completedAt` desc; card hidden entirely if no prior session; `client/components/SessionComparisonCard.tsx`
- **2026-05-14**: Training streak & monthly summary cards — Progress home screen now shows a Streak card (current + longest consecutive training day streak, zap icon, "Personal best" label when at record) and a Monthly Comparison card (Sessions, Training Time, Volume, Distance vs prior month with % delta arrows); both load all sessions/tasks from storage on focus and refresh; `client/components/StreakCard.tsx` exports `computeStreaks` helper; `client/components/MonthlyComparisonCard.tsx` exports `computeMonthStats` helper
- **2026-05-14**: Long-term progress charts — new "Long-Term Progress" screen accessible via bar-chart-2 icon in Progress header; five chart sections: Workout Frequency (bar), Strength Progression (line, per exercise, tap-to-tooltip), Weekly Volume (bar), Distance/Cardio (line), Body Weight (line); each section has 1M/3M/6M/All range toggles; built with react-native-svg; data helpers in `client/lib/chartData.ts`; chart components in `client/components/charts/`
- **2026-04-06**: Reference link import from Excel/CSV — added `reference_link` column mapping to import flow; auto-detects columns named "reference_link", "link", "url", or "video"; parsed links passed to `taskTemplatesStorage.create`; link icon shown in preview step; strength and endurance CSV templates updated with `reference_link` column
- **2026-06-15**: Global previous performance hints — "Last: X kg × Y reps" hints during workouts now match by exercise name across all plans and days, not just the same task template; single `getAll()` load grouped by name (case-insensitive), most recent completed session used; `client/screens/SessionRunScreen.tsx` `loadPreviousData()`
- **2026-05-14**: Exercise reorder within a day — confirmed working via automated e2e test; expand any exercise card to see up/down arrow buttons; exercises can be moved up or down within the same training day; uses existing `reorderTasks` storage logic; first exercise hides up arrow, last hides down arrow
- **2026-03-05**: Codebase audit fixes — fixed `formatRelativeDate` to use calendar-based date diff (yesterday bug); fixed minute stepper overflow into hour on Weight Reminder screen; defaulted `distanceUnit` to "km" in Program Builder; removed unused imports (`TaskTemplate` in AddTaskScreen, `loading` in SettingsScreen, `Spacing` in WeeklyStatsCard); removed dead storage functions (`getByWeek`, `badgesStorage.create`, `badgesStorage.getByType`); fixed console.log → console.error in TaskCard
